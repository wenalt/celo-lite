...
const ETHERSCAN_KEY =
  ...
  process.env.NEXT_PUBLIC_ETHERSCAN_API_KEY || // fallback si tu souhaites tester vite, à éviter en prod
  "";
  ...
  try {
    ...
    while (true) {
      ...
      for (const tx of data.result) {
        total++;
        if (Number(tx.timeStamp) >= CELO_L2_START_TS) l2++;
      }
    ...
    return res.status(200).json({ l1, l2, total });
  } catch (e) {
